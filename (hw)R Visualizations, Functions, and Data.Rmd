...
#to instsall packages/data sets
...
```{r}
#filter on the information in the dataframe, where the month ==3, day ==31
...
```{r}
#carrier is a column i'm referencing
carrier_df <- filter(flights, carrier=='AA' | carrier=='DL' | carrier == 'UA')
carrier_df
```

```{r}
#using filter to create a table to show just the information i want to see
...
#slect all colums from the flights data set that start with the letter c
select(flights, starts_with('c'))
select(flights, ends_with('e'))
select(flights, contains('g'))
select(flights, matches('fl|c')) #regex expression
...
select(flights, 2,4,7)
#commas specifies specific columns
```
...
#transmute only returns the new stuff yu built
```

### Summarizing Data

```{r}
summarize(flight, delay = mean(dep_delay, na.rm=TRUE))
```
...
select(mtcars, matches('mp|c')) #regex expression
...
by_displ <- group_by(mtcars_data_mut)
summarize(by_displ, force=mean(new_column, na.rm=TRUE))
```
...
delay <-filter(delay, count > 20, dest != 'HNL') #honolulu gone now
delay
```
...
```{r}
#if its not an na i don't want it, so it will do the opposite of get ride of null value
...
ggplot(data=myplot)+
  ...
  axis.text.x =element_text(size=10, color='red'),
...
#jitter allows for a bit more noise to be present
```
...
```{r}
#create a function that could convert celcius to farenheight
#c * (9/5) + 32
c_to_f < function(val){
...
calc_factorial <- function(x){
  if(x < 0){
    print('factorial is ot defined for negative numbers')
  }else if (x==0){
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    ```{r}
    # Problem 1 I solved in R: Multiplication
    # Write the solution: 
    mul <- function(a, b) {
      a*b
    }
    # Problem 2 I solved in R: return negative number
    # Write the solution: 
    make_negative <- function(num){
      return(-abs(num))
    }

    # Problem 3 I solved in R:Convert a Number to a String!
    number_to_string <- function(n) {
      n = as.character(n)
      print(n)
    } 
    # Problem 4 I solved in R:Reversed Strings
    solution <- function(s){
...
-   Null Hypothesis:There is no significant difference between crime and
    the growth of population
-   Alternative Hypothesis: As the population grows so will the amount
    of crime.
